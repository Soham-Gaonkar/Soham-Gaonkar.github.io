...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="w-full h-24 flex items-center px-4 md:px-12 font-sans bg-transparent">
      <div className="max-w-screen-2xl mx-auto flex items-center justify-between w-full">
        {/* Left: Name */}
        <div className="text-3xl md:text-4xl font-medium text-black dark:text-white tracking-tight whitespace-nowrap" style={{ fontFamily: 'Inter, DM Sans, sans-serif' }}>
          Soham Gaonkar
        </div>
        {/* Right: Nav Links, Social Icons, Dark Mode */}
          ...
          <div className="flex items-center gap-2">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className={`text-2xl font-semibold px-6 py-1 transition-all duration-300 border-b-2 border-transparent
